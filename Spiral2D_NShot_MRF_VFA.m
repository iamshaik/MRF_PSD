...
for itr = 1:length(FA_16shots)
  [rf, gz] = mr.makeSincPulse(round(FA_16shots(itr)),system,'Duration',1e-3+0.05e-3.*itr,...
 'SliceThickness',sliceThickness,'apodization',0.5,'timeBwProduct',4);  
    rf_all(itr) = rf;
    gz_all(itr) = gz; 
end
...
fname = [date,'_Spiral_2DMRF_varTR',num2str(Nshots),'_',num2str(alpha),'_',num2str(system.maxSlew/gamma),'_',num2str(Gmax),'_',num2str(fov*1e3),'_'];
save(fname, 'ktrajs' );
%% Write to file
